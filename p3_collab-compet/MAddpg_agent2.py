...
class MADDPG_Agent():
    ...
    def __init__(self, state_size, action_size, num_agent, random_seed):
        ...
        # tracking update trigger count, will be reset to 0 when update occur
        self.t = 0
        
        # agent that begins the training
        self.order = 0
        ...
        if len(self.memory) > BATCH_SIZE and (self.t % UPDATE_EVERY) == 0:
            ...
            for _ in range(LEARN_STEPS):
                ...
                for i in range(self.num_agent):
                    agent_id = (i+self.order)%self.num_agent
                    experiences = self.memory.sample()
                    self.learn(experiences, GAMMA, agent_id)
                    
                # set next agent which starts the training process
                if self.order+1 < self.num_agent:
            ...
            # reset internal update trigger count
            self.t = 0
